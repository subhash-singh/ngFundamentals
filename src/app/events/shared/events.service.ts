...
export class EventsService {
 ...
 searchSessions(searchTearm: string) {
   const term = searchTearm.toLocaleLowerCase();
   let results: ISession[] = [] ;
   EVENTS.forEach(event => {
    let machingSessions = event.sessions.filter( session => session.name.toLowerCase().indexOf(term) > -1);
    ...
    setTimeout(() => { emmiter.emit(results); }
    , 100);
